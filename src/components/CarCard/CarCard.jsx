...
import { CarItem, Item, ImgCar, Info, Titel,Model, Description, Button, FavoritesButton } from './CarCard.styled';
...
// const [isCheckCar, setIsCheckCar] = useState(false)
...
const togleFavoriteList = () => {
  ...
  return (
    <CarItem>
      <Item>
        <FavoritesButton src={checkCar ? active : normal} alt='heart' onClick={togleFavoriteList}/>
        <ImgCar src={carFoto} alt='foto'/>
        <Info>
            <Titel><p>{car.make}<Model>{car.make==='Volvo' || car.make==='Buick'? car.model:''}</Model>,{car.year}</p>  <p>{car.rentalPrice}</p></Titel>
            <Description>
                <span><p style={{paddingLeft: '0px'}}>{city}</p> <p>{coutry}</p> <p>{car.rentalCompany}</p> <p>{premium}</p></span>
                 
                <span><p style={{paddingLeft: '0px'}}>{car.type}</p> <p>{car.model}</p>  <p>{car.id}</p> </span>
